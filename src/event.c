...
#define DO_EVENT_HOOK_CALLBACK(type, xcbtype, xcbeventprefix, arraytype, match)              \
    static void event_##xcbtype##_callback(                                                  \
        xcb_##xcbtype##_press_event_t *ev, arraytype *arr, lua_State *L, int oud, int nargs, \
        void *data) {                                                                        \
        int abs_oud       = oud < 0 ? ((lua_gettop(L) + 1) + oud) : oud;                     \
        int item_matching = 0;                                                               \
        foreach (item, *arr)                                                                 \
            if (match(ev, *item, data)) {                                                    \
                if (oud) luna_object_push_item(L, abs_oud, *item);                           \
                else luna_object_push(L, *item);                                             \
                item_matching++;                                                             \
            }                                                                                \
        for (; item_matching > 0; item_matching--) {                                         \
            switch (ev->response_type) {                                                     \
                case xcbeventprefix##_PRESS:                                                 \
                    ...
                    luna_object_emit_signal(L, -nargs - 1, "press", nargs);                  \
                    break;                                                                   \
                case xcbeventprefix##_RELEASE:                                               \
                    for (int i = 0; i < nargs; i++)                                          \
                        lua_pushvalue(L, -nargs - item_matching);                            \
                    luna_object_emit_signal(L, -nargs - 1, "release", nargs);                \
                    break;                                                                   \
            }                                                                                \
            lua_pop(L, 1);                                                                   \
        }                                                                                    \
...
static bool event_key_match(xcb_key_press_event_t *ev, keyb_t *k, void *data) {
    ...
    return (
        ((k->keycode && ev->detail == k->keycode) || (k->keysym && keysym == k->keysym)) &&
...
static bool event_handle_mousegrabber(int x, int y, uint16_t mask) {
    if (globalconf.mousegrabber != LUA_REFNIL) {
        ...
        if (!luaA_dofunction(L, 1, 1)) {
            ...
            if (!lua_isboolean(L, -1) || !lua_toboolean(L, -1)) luaA_mousegrabber_stop(L);
...
static void event_emit_button(lua_State *L, xcb_button_press_event_t *ev) {
    ...
    /* And emit the signal */
    luna_object_emit_signal(L, -5, name, 4);
}
...
static void event_handle_button(xcb_button_press_event_t *ev) {
...

    {
        ...
        if (XCB_EVENT_RESPONSE_TYPE(ev) == XCB_BUTTON_PRESS) state |= change;
        else state &= ~change;
        if (event_handle_mousegrabber(ev->root_x, ev->root_y, state)) return;
    }
    ...
    if ((drawin = drawin_getbywin(ev->event)) || (drawin = drawin_getbywin(ev->child))) {
        ...
        if (ev->child == XCB_NONE)
            xcb_allow_events(globalconf.connection, XCB_ALLOW_ASYNC_POINTER, ev->time);
        ...
        xcb_allow_events(globalconf.connection, XCB_ALLOW_REPLAY_POINTER, ev->time);
    ...
    if ((c = client_getbywin(ev->window))) {
        ...
        if (c->size_hints.flags & XCB_ICCCM_SIZE_HINT_P_WIN_GRAVITY) {
            xwindow_translate_for_gravity(
                c->size_hints.win_gravity, 0, 0, diff_w, diff_h, &geometry.x, &geometry.y);
        }
        if (ev->value_mask & XCB_CONFIG_WINDOW_X) {
            geometry.x = ev->x;
            if (c->size_hints.flags & XCB_ICCCM_SIZE_HINT_P_WIN_GRAVITY)
                xwindow_translate_for_gravity(
                    c->size_hints.win_gravity, deco_left, 0, deco_right, 0, &geometry.x, NULL);
        }
        if (ev->value_mask & XCB_CONFIG_WINDOW_Y) {
            geometry.y = ev->y;
            if (c->size_hints.flags & XCB_ICCCM_SIZE_HINT_P_WIN_GRAVITY)
                xwindow_translate_for_gravity(
                    c->size_hints.win_gravity, 0, deco_top, 0, deco_bottom, NULL, &geometry.y);
        }
        ...
        xcb_translate_coordinates_cookie_t coords_cookie = xcb_translate_coordinates_unchecked(
            globalconf.connection, ev->window, globalconf.screen->root, 0, 0);
        ...
        p_delete(&geom);
        p_delete(&coords);
    } else event_handle_configurerequest_configure_window(ev);
}
...
static void event_handle_configurenotify(xcb_configure_notify_event_t *ev) {
    ...
    if (ev->window == screen->root) screen_schedule_refresh();
...
static void event_handle_destroynotify(xcb_destroy_notify_event_t *ev) {
    ...
    if ((c = client_getbywin(ev->window))) client_unmanage(c, CLIENT_UNMANAGE_DESTROYED);
    else
...
void event_drawable_under_mouse(lua_State *L, int ud) {
    ...
    lua_pushvalue(L, ud);
    d = luna_object_ref(L, -1);

    if (d == globalconf.drawable_under_mouse) {
        /* Nothing to do */
        luna_object_unref(L, d);
        return;
    }

    if (globalconf.drawable_under_mouse != NULL) {
        /* Emit leave on previous drawable */
        luna_object_push(L, globalconf.drawable_under_mouse);
        luna_object_emit_signal(L, -1, ":mouse.leave", 0);
        lua_pop(L, 1);

        /* Unref the previous drawable */
        luna_object_unref(L, globalconf.drawable_under_mouse);
    ...
    if (d != NULL) {
        ...
        /* Emit enter */
        luna_object_emit_signal(L, ud, ":mouse.enter", 0);
    }
}
...
static void event_handle_motionnotify(xcb_motion_notify_event_t *ev) {
    ...
    if (event_handle_mousegrabber(ev->root_x, ev->root_y, ev->state)) return;

    if ((c = client_getbyframewin(ev->event))) {
        luna_object_push(L, c);
        lua_pushinteger(L, ev->event_x);
        lua_pushinteger(L, ev->event_y);
        luna_object_emit_signal(L, -3, ":mouse.move", 2);
    ...
    if ((w = drawin_getbywin(ev->event))) {
        luna_object_push(L, w);
        luna_object_push_item(L, -1, w->drawable);
        event_drawable_under_mouse(L, -1);
        lua_pushinteger(L, ev->event_x);
        lua_pushinteger(L, ev->event_y);
        luna_object_emit_signal(L, -3, ":mouse.move", 2);
...
static void event_handle_leavenotify(xcb_leave_notify_event_t *ev) {
     ...
     * grab.
     */
    if (ev->mode != XCB_NOTIFY_MODE_NORMAL) return;
...
static void event_handle_enternotify(xcb_enter_notify_event_t *ev) {
    ...
    if ((c = client_getbyframewin(ev->event))) {
         ...
         */
        if (ev->detail != XCB_NOTIFY_DETAIL_INFERIOR) {
            luna_object_emit_signal(L, -1, ":mouse.enter", 0);
        }
...
static void event_handle_key(xcb_key_press_event_t *ev) {
    ...
    if (globalconf.keygrabber != LUA_REFNIL) {
        ...
        } else event_key_callback(ev, &globalconf.keys, L, 0, 0, &keysym);
    }
}
...
static void event_handle_maprequest(xcb_map_request_event_t *ev) {
    ...
    if (!(wa_r = xcb_get_window_attributes_reply(globalconf.connection, wa_c, NULL))) return;

    if (wa_r->override_redirect) goto bailout;
...
static void event_handle_unmapnotify(xcb_unmap_notify_event_t *ev) {
    ...
    if ((c = client_getbywin(ev->window))) client_unmanage(c, CLIENT_UNMANAGE_UNMAP);
}
...
static void event_handle_randr_screen_change_notify(xcb_randr_screen_change_notify_event_t *ev) {
    ...
    if (ev->root != globalconf.screen->root) return;
...
static void event_handle_randr_output_change_notify(xcb_randr_notify_event_t *ev) {
    if (ev->subCode == XCB_RANDR_NOTIFY_OUTPUT_CHANGE) {
        ...
        info                                 = xcb_randr_get_output_info_reply(
            ...
            xcb_randr_get_output_info_unchecked(globalconf.connection, output, XCB_CURRENT_TIME),
            NULL);
        if (!info) return;
        ...
        lua_pushlstring(
            L, (char *)xcb_randr_get_output_info_name(info),
            xcb_randr_get_output_info_name_length(info));
        lua_pushstring(L, connection_str);
        luna_emit_global_signal(L, ":screen.change", 2);
...
static void event_handle_shape_notify(xcb_shape_notify_event_t *ev) {
    ...
    if (c) {
        ...
        if (ev->shape_kind == XCB_SHAPE_SK_BOUNDING)
            luna_object_emit_signal(L, -1, ":property.shape_client_bounding", 0);
        if (ev->shape_kind == XCB_SHAPE_SK_CLIP)
            luna_object_emit_signal(L, -1, ":property.shape_client_clip", 0);
        lua_pop(L, 1);
    }
}
...
static void event_handle_clientmessage(xcb_client_message_event_t *ev) {
    ...
    if (sn_xcb_display_process_event(globalconf.sndisplay, (xcb_generic_event_t *)ev)) return;
    ...
    } else if (ev->type == _XEMBED) xembed_process_client_message(ev);
    else if (ev->type == _NET_SYSTEM_TRAY_OPCODE) systray_process_client_message(ev);
    else ewmh_process_client_message(ev);
}
...
static void event_handle_reparentnotify(xcb_reparent_notify_event_t *ev) {
    ...
    if ((c = client_getbywin(ev->window)) && c->frame_window != ev->parent) {
        ...
        if (ev->parent != globalconf.screen->root) client_unmanage(c, CLIENT_UNMANAGE_REPARENT);
...
static void event_handle_selectionclear(xcb_selection_clear_event_t *ev) {
    ...
    } else selection_handle_selectionclear(ev);
}
...
static void xerror(xcb_generic_error_t *e) {
    ...
    const char *major = xcb_errors_get_name_for_major_code(globalconf.errors_ctx, e->major_code);
    const char *minor =
        xcb_errors_get_name_for_minor_code(globalconf.errors_ctx, e->major_code, e->minor_code);
    const char *extension = NULL;
    const char *error =
        xcb_errors_get_name_for_error(globalconf.errors_ctx, e->error_code, &extension);
#else
...
#endif
    warn(
        "X error: request=%s%s%s (major %d, minor %d), error=%s%s%s (%d)", major,
        minor == NULL ? "" : "-", NONULL(minor), e->major_code, e->minor_code, NONULL(extension),
        extension == NULL ? "" : "-", error, e->error_code);
...
static bool should_ignore(xcb_generic_event_t *event) {
    ...
    while (globalconf.ignore_enter_leave_events.len > 0) {
         ...
         */
        if (end - sequence < UINT32_MAX / 2) break;
    ...
    if ((response_type == XCB_ENTER_NOTIFY || response_type == XCB_LEAVE_NOTIFY) &&
        ...
        uint32_t end   = globalconf.ignore_enter_leave_events.tab[0].end.sequence;
        if (sequence >= begin && sequence <= end) return true;
    }
...
void event_init(void) {
    ...
    if (reply && reply->present) globalconf.event_base_randr = reply->first_event;

    reply = xcb_get_extension_data(globalconf.connection, &xcb_shape_id);
    if (reply && reply->present) globalconf.event_base_shape = reply->first_event;

    reply = xcb_get_extension_data(globalconf.connection, &xcb_xkb_id);
    if (reply && reply->present) globalconf.event_base_xkb = reply->first_event;

    reply = xcb_get_extension_data(globalconf.connection, &xcb_xfixes_id);
    if (reply && reply->present) globalconf.event_base_xfixes = reply->first_event;
}
