...
#define foreach(var, array)                                                                            \
    for (int __foreach_index_##var = 0; __foreach_index_##var < (array).len;                           \
        ...
        for (__typeof__((array).tab) var = &(array).tab[__foreach_index_##var];                    \
             (__foreach_index_##var < (array).len) && (var = &(array).tab[__foreach_index_##var]); \
             ++__foreach_index_##var)

#define foreach_reverse(var, array)                                                           \
    for (int __foreach_index_##var = (array).len - 1; __foreach_index_##var > -1;             \
         __foreach_index_##var     = -1)                                                      \
        for (__typeof__((array).tab) var = &(array).tab[__foreach_index_##var];           \
             (__foreach_index_##var > -1) && (var = &(array).tab[__foreach_index_##var]); \
             --__foreach_index_##var)

/** Common array functions */
#define ARRAY_COMMON_FUNCS(type_t, pfx, dtor)                                                        \
    static inline pfx##_array_t *pfx##_array_new(void) { return p_new(pfx##_array_t, 1); }           \
    static inline void           pfx##_array_init(pfx##_array_t *arr) { p_clear(arr, 1); }           \
    ...
    static inline void pfx##_array_splice(                                                           \
        pfx##_array_t *arr, int pos, int len, type_t items[], int count) {                           \
    ...
    static inline int    pfx##_array_indexof(pfx##_array_t *arr, type_t *e) { return e - arr->tab; } \
...
#define BARRAY_FUNCS(type_t, pfx, dtor, cmp)                                  \
    ARRAY_COMMON_FUNCS(type_t, pfx, dtor)                                     \
    static inline void pfx##_array_insert(pfx##_array_t *arr, type_t e) {     \
        int l = 0, r = arr->len;                                              \
        while (l < r) {                                                       \
            int i   = (r + l) / 2;                                            \
    ...
    static inline type_t *pfx##_array_lookup(pfx##_array_t *arr, type_t *e) { \
