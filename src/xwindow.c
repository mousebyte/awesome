...
void xwindow_set_state(xcb_window_t win, uint32_t state) {
    ...
    xcb_change_property(
        globalconf.connection, XCB_PROP_MODE_REPLACE, win, WM_STATE, WM_STATE, 32, 2, data);
}
...
xcb_get_property_cookie_t xwindow_get_state_unchecked(xcb_window_t w) {
    return xcb_get_property_unchecked(globalconf.connection, false, w, WM_STATE, WM_STATE, 0L, 2L);
}
 ...
 */
void xwindow_buttons_grab(xcb_window_t win, button_array_t *buttons) {
    ...
    foreach (b, *buttons)
        xcb_grab_button(
            globalconf.connection, false, win, BUTTONMASK, XCB_GRAB_MODE_SYNC, XCB_GRAB_MODE_ASYNC,
            XCB_NONE, XCB_NONE, (*b)->button, (*b)->modifiers);
}
 ...
 * \param k The key.
 */
static void xwindow_grabkey(xcb_window_t win, keyb_t *k) {
    if (k->keycode)
        xcb_grab_key(
            globalconf.connection, true, win, k->modifiers, k->keycode, XCB_GRAB_MODE_ASYNC,
            XCB_GRAB_MODE_ASYNC);
    else if (k->keysym) {
        ...
        if (keycodes) {
            for (xcb_keycode_t *kc = keycodes; *kc; kc++)
                xcb_grab_key(
                    globalconf.connection, true, win, k->modifiers, *kc, XCB_GRAB_MODE_ASYNC,
                    XCB_GRAB_MODE_ASYNC);
            p_delete(&keycodes);
        }
    }
...
void xwindow_grabkeys(xcb_window_t win, key_array_t *keys) {
    /* Ungrab everything first */
...
xcb_get_property_cookie_t xwindow_get_opacity_unchecked(xcb_window_t win) {
    return xcb_get_property_unchecked(
        globalconf.connection, false, win, _NET_WM_WINDOW_OPACITY, XCB_ATOM_CARDINAL, 0L, 1L);
}
...
double xwindow_get_opacity_from_cookie(xcb_get_property_cookie_t cookie) {
    xcb_get_property_reply_t *prop_r = xcb_get_property_reply(globalconf.connection, cookie, NULL);
...
void xwindow_set_opacity(xcb_window_t win, double opacity) {
    if (win) {
        if (opacity >= 0 && opacity <= 1) {
            ...
            xcb_change_property(
                globalconf.connection, XCB_PROP_MODE_REPLACE, win, _NET_WM_WINDOW_OPACITY,
                XCB_ATOM_CARDINAL, 32, 1L, &real_opacity);
        } else xcb_delete_property(globalconf.connection, win, _NET_WM_WINDOW_OPACITY);
    }
}
...
void xwindow_takefocus(xcb_window_t win) {
    ...
    xcb_send_event(globalconf.connection, false, win, XCB_EVENT_MASK_NO_EVENT, (char *)&ev);
}
...
void xwindow_set_cursor(xcb_window_t w, xcb_cursor_t c) {
    xcb_change_window_attributes(globalconf.connection, w, XCB_CW_CURSOR, (const uint32_t[]) {c});
}
...
cairo_surface_t *xwindow_get_shape(xcb_window_t win, enum xcb_shape_sk_t kind) {
    if (!globalconf.have_shape) return NULL;
    if (kind == XCB_SHAPE_SK_INPUT && !globalconf.have_input_shape) return NULL;

    int16_t                           x, y;
    uint16_t                          width, height;
    xcb_shape_get_rectangles_cookie_t rcookie =
        xcb_shape_get_rectangles(globalconf.connection, win, kind);
    if (kind == XCB_SHAPE_SK_INPUT) {
        /* We cannot query the size/existence of an input shape... */
        xcb_get_geometry_reply_t *geom = xcb_get_geometry_reply(
            globalconf.connection, xcb_get_geometry(globalconf.connection, win), NULL);
        if (!geom) {
        ...
        xcb_shape_query_extents_cookie_t ecookie =
            xcb_shape_query_extents(globalconf.connection, win);
        xcb_shape_query_extents_reply_t *extents =
            xcb_shape_query_extents_reply(globalconf.connection, ecookie, NULL);
        bool shaped;
    ...
    xcb_shape_get_rectangles_reply_t *rects_reply =
        xcb_shape_get_rectangles_reply(globalconf.connection, rcookie, NULL);
    if (!rects_reply) {
        /* Create a cairo surface in an error state */
        return cairo_image_surface_create(CAIRO_FORMAT_INVALID, -1, -1);
    }
...
static xcb_pixmap_t xwindow_shape_pixmap(int width, int height, cairo_surface_t *surf) {
    ...
    if (width <= 0 || height <= 0) return XCB_NONE;
...
void xwindow_set_shape(
    ...
    int                 offset) {
    if (!globalconf.have_shape) return;
    if (kind == XCB_SHAPE_SK_INPUT && !globalconf.have_input_shape) return;

    xcb_pixmap_t pixmap = XCB_NONE;
    if (surf) pixmap = xwindow_shape_pixmap(width, height, surf);

    xcb_shape_mask(globalconf.connection, XCB_SHAPE_SO_SET, kind, win, offset, offset, pixmap);

    if (pixmap != XCB_NONE) xcb_free_pixmap(globalconf.connection, pixmap);
}
