...
static void root_set_wallpaper_pixmap(xcb_connection_t *c, xcb_pixmap_t p) {
    ...
    prop_c =
        xcb_get_property_unchecked(c, false, screen->root, ESETROOT_PMAP_ID, XCB_ATOM_PIXMAP, 0, 1);
     ...
     */
    xcb_change_property(
        c, XCB_PROP_MODE_REPLACE, screen->root, _XROOTPMAP_ID, XCB_ATOM_PIXMAP, 32, 1, &p);
    xcb_change_property(
        c, XCB_PROP_MODE_REPLACE, screen->root, ESETROOT_PMAP_ID, XCB_ATOM_PIXMAP, 32, 1, &p);
    ...
    if (prop_r && prop_r->value_len) {
        ...
        if (rootpix) xcb_kill_client(c, *rootpix);
    }
    p_delete(&prop_r);
}
...
static bool root_set_wallpaper(cairo_pattern_t *pattern) {
    ...
    if (xcb_connection_has_error(c)) goto disconnect;
    ...
    surface = cairo_xcb_surface_create(
        globalconf.connection, p, draw_default_visual(screen), width, height);
    ...
    xcb_change_window_attributes(
        globalconf.connection, globalconf.screen->root, XCB_CW_EVENT_MASK, (uint32_t[]) {0});
    ...
    luna_emit_global_signal(L, "wallpaper_changed", 0);
...
void root_update_wallpaper(void) {
    ...
    prop_c               = xcb_get_property_unchecked(
        globalconf.connection, false, globalconf.screen->root, _XROOTPMAP_ID, XCB_ATOM_PIXMAP, 0,
        1);
    ...
    if (geom_r->depth != draw_visual_depth(globalconf.screen, globalconf.default_visual->visual_id))
        warn(
            "Got a pixmap with depth %d, but the default depth is %d, continuing anyway",
            geom_r->depth,
            draw_visual_depth(globalconf.screen, globalconf.default_visual->visual_id));

    globalconf.wallpaper = cairo_xcb_surface_create(
        globalconf.connection, *rootpix, globalconf.default_visual, geom_r->width, geom_r->height);
...
static int luaA_root_fake_input(lua_State *L) {
    ...
    xcb_test_fake_input(
        globalconf.connection, type, detail, 0, /* This is a delay, not a timestamp! */
        XCB_NONE, x, y, 0);
    return 0;
}
...
static int luaA_root_keys(lua_State *L) {
    if (lua_gettop(L) == 1) {
        ...
        foreach (key, globalconf.keys)
            luna_object_unref(L, *key);

        key_array_wipe(&globalconf.keys);
        key_array_init(&globalconf.keys);

        // TODO: typecheck here. examine how we want to create keys in user code
        lua_pushnil(L);
        while (lua_next(L, 1))
            key_array_append(&globalconf.keys, luna_object_ref(L, -1));
    ...
    for (int i = 0; i < globalconf.keys.len; i++) {
        luna_object_push(L, globalconf.keys.tab[i]);
        lua_rawseti(L, -2, i + 1);
    }
...
static int luaA_root_buttons(lua_State *L) {
    if (lua_gettop(L) == 1) {
        ...
        lua_pushnil(L);
        while (lua_next(L, 1))
            button_array_append(&globalconf.buttons, luna_object_ref(L, -1));
    ...
    for (int i = 0; i < globalconf.buttons.len; i++) {
        luna_object_push(L, globalconf.buttons.tab[i]);
        lua_rawseti(L, -2, i + 1);
    }
...
static int luaA_root_cursor(lua_State *L) {
    ...
    if (cursor_font) {
        ...
        xcb_change_window_attributes(
            globalconf.connection, globalconf.screen->root, XCB_CW_CURSOR, change_win_vals);
    } else luaA_warn(L, "invalid cursor %s", cursor_name);
...
static int luaA_root_drawins(lua_State *L) {
    ...
    for (int i = 0; i < globalconf.drawins.len; i++) {
        luna_object_push(L, globalconf.drawins.tab[i]);
        lua_rawseti(L, -2, i + 1);
    }
...
static int luaA_root_wallpaper(lua_State *L) {
    ...
    if (globalconf.wallpaper == NULL) return 0;
...
static int luaA_root_get_content(lua_State *L) {
    ...
    surface = cairo_xcb_surface_create(
        globalconf.connection, globalconf.screen->root, globalconf.default_visual,
...
static int luaA_root_tags(lua_State *L) {
    ...
    for (int i = 0; i < globalconf.tags.len; i++) {
        luna_object_push(L, globalconf.tags.tab[i]);
        lua_rawseti(L, -2, i + 1);
    }
...
static int luaA_root_index(lua_State *L) {
    if (miss_index_handler != LUA_REFNIL) return luaA_call_handler(L, miss_index_handler);
...
static int luaA_root_newindex(lua_State *L) {
    ...
    if (miss_newindex_handler != LUA_REFNIL) return luaA_call_handler(L, miss_newindex_handler);
