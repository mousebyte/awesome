...
#define REGISTRY_TRANSFER_TABLE_INDEX "luna_selection_transfers"
...
static void selection_transfer_notify(
    xcb_window_t    requestor,
    xcb_atom_t      selection,
    xcb_atom_t      target,
    xcb_atom_t      property,
...
void selection_transfer_reject(
    xcb_window_t    requestor,
    xcb_atom_t      selection,
    xcb_atom_t      target,
    xcb_timestamp_t time) {
...
static void transfer_continue_incremental(lua_State *L, int ud) {
    const char           *data;
    size_t                data_length;
    selection_transfer_t *transfer = luaC_checkuclass(L, ud, "SelectionTransfer");
    ...
    if (transfer->offset == data_length) {
        if (transfer->more_data) {
            ...
            luna_object_emit_signal(L, ud, "continue", 0);
        ...
        /* End of transfer */
        xcb_change_property(
            globalconf.connection, XCB_PROP_MODE_REPLACE, transfer->requestor, transfer->property,
            UTF8_STRING, 8, 0, NULL);
        xcb_change_window_attributes(
            globalconf.connection, transfer->requestor, XCB_CW_EVENT_MASK, (uint32_t[]) {0});
        ...
        xcb_change_property(
            globalconf.connection, XCB_PROP_MODE_REPLACE, transfer->requestor, transfer->property,
            UTF8_STRING, 8, next_length, &data[transfer->offset]);
...
void selection_transfer_begin(
    ...
    luaC_construct(L, 0, "SelectionTransfer");
    ...
    xcb_get_atom_name_reply_t *reply = xcb_get_atom_name_reply(
        globalconf.connection, xcb_get_atom_name_unchecked(globalconf.connection, target), NULL);
    if (reply) {
    ...
    lua_pushvalue(L, -2);
    luna_object_emit_signal(L, ud, "request", 2);
...
static int luaA_selection_transfer_send(lua_State *L) {
    ...
    selection_transfer_t *transfer = luaC_checkuclass(L, 1, "SelectionTransfer");
    ...
    if (incr && lua_isnumber(L, -1)) incr_size = lua_tonumber(L, -1);
    lua_pop(L, 1);
    ...
    if (lua_isstring(L, -2)) {
        ...
        if (A_STRNEQ(format_string, "atom")) luaL_error(L, "Unknown format '%s'", format_string);
        if (incr) luaL_error(L, "Cannot transfer atoms in pieces");
        ...
        for (size_t i = 0; i < len; i++) {
            cookies[i] = xcb_intern_atom_unchecked(
                globalconf.connection, false, atom_lengths[i], atom_strings[i]);
        }
        for (size_t i = 0; i < len; i++) {
            xcb_intern_atom_reply_t *reply =
                xcb_intern_atom_reply(globalconf.connection, cookies[i], NULL);
        ...
        xcb_change_property(
            globalconf.connection, XCB_PROP_MODE_REPLACE, transfer->requestor, transfer->property,
            XCB_ATOM_ATOM, 32, len, &atoms[0]);
    } else {
        /* 'data' is a string with the data to transfer */
        const char *data = luaL_checklstring(L, -1, &data_length);

        if (!incr) incr_size = data_length;

        if (data_length >= max_property_length()) incr = true;

        if (incr) {
            xcb_change_window_attributes(
                globalconf.connection, transfer->requestor, XCB_CW_EVENT_MASK,
                (uint32_t[]) {XCB_EVENT_MASK_PROPERTY_CHANGE});

            xcb_change_property(
                globalconf.connection, XCB_PROP_MODE_REPLACE, transfer->requestor,
            ...
            xcb_change_property(
                globalconf.connection, XCB_PROP_MODE_REPLACE, transfer->requestor,
                transfer->property, UTF8_STRING, 8, data_length, data);
        }
    }

    selection_transfer_notify(
        transfer->requestor, transfer->selection, transfer->target, transfer->property,
        transfer->time);
    if (!incr) transfer_done(L, transfer);
...
void selection_transfer_handle_propertynotify(xcb_property_notify_event_t *ev) {
    ...
    if (ev->state != XCB_PROPERTY_DELETE) return;
...
static luaL_Reg selection_transfer_methods[] = {
    {"send", luaA_selection_transfer_send},
    {NULL,   NULL                        }
};

static luaC_Class selection_transfer_class = {
    .name      = "SelectionTransfer",
    .parent    = "Object",
    .user_ctor = 0,
    .alloc     = lunaL_selection_transfer_alloc,
    .gc        = NULL,
    .methods   = selection_transfer_methods};

void luaC_register_selection_transfer(lua_State *L) {
    lua_pushlightuserdata(L, &selection_transfer_class);
    luaC_register(L, -1);
