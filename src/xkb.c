...
int luaA_xkb_set_layout_group(lua_State *L) {
    ...
    xcb_xkb_latch_lock_state(
        globalconf.connection, XCB_XKB_ID_USE_CORE_KBD, 0, 0, true, group, 0, 0, 0);
    return 0;
}
...
int luaA_xkb_get_layout_group(lua_State *L) {
    ...
    state_c = xcb_xkb_get_state_unchecked(globalconf.connection, XCB_XKB_ID_USE_CORE_KBD);
    xcb_xkb_get_state_reply_t *state_r;
    state_r = xcb_xkb_get_state_reply(globalconf.connection, state_c, NULL);
    if (!state_r) {
        free(state_r);
        return 0;
    }
...
int luaA_xkb_get_group_names(lua_State *L) {
    ...
    name_c = xcb_xkb_get_names_unchecked(
        globalconf.connection, XCB_XKB_ID_USE_CORE_KBD, XCB_XKB_NAME_DETAIL_SYMBOLS);
    ...
    xcb_xkb_get_names_value_list_unpack(
        buffer, name_r->nTypes, name_r->indicators, name_r->virtualMods, name_r->groupNames,
        name_r->nKeys, name_r->nKeyAliases, name_r->nRadioGroups, name_r->which, &name_list);
...
static bool fill_rmlvo_from_root(struct xkb_rule_names *xkb_names) {
    xcb_get_property_reply_t *prop_reply = xcb_get_property_reply(
        ...
        xcb_get_property_unchecked(
            globalconf.connection, false, globalconf.screen->root, _XKB_RULES_NAMES,
...
static void xkb_fill_state(void) {
    ...
    if (globalconf.have_xkb) {
        ...
        if (device_id == -1) warn("Failed while getting XKB device id");
    }

    if (device_id != -1) {
        struct xkb_keymap *xkb_keymap = xkb_x11_keymap_new_from_device(
            globalconf.xkb_ctx, conn, device_id, XKB_KEYMAP_COMPILE_NO_FLAGS);

        if (!xkb_keymap) fatal("Failed while getting XKB keymap from device");

        globalconf.xkb_state = xkb_x11_state_new_from_device(xkb_keymap, conn, device_id);
        if (!globalconf.xkb_state) fatal("Failed while getting XKB state from device");
        ...
        if (!globalconf.xkb_state) fatal("Failed while creating XKB state");
...
static void xkb_init_keymap(void) {
    ...
    if (!globalconf.xkb_ctx) fatal("Failed while getting XKB context");
...
static gboolean xkb_refresh(gpointer unused) {
    ...
    if (globalconf.xkb_reload_keymap) xkb_reload_keymap();
    if (globalconf.xkb_map_changed) luna_emit_global_signal(L, ":xkb.map_changed", 0);
    if (globalconf.xkb_group_changed) luna_emit_global_signal(L, ":xkb.group_changed", 0);
...
static void xkb_schedule_refresh(void) {
    if (globalconf.xkb_update_pending) return;
    ...
    switch (event->pad0) {
        ...
        case XCB_XKB_STATE_NOTIFY: {
            ...
            xkb_state_update_mask(
                globalconf.xkb_state, state_notify_event->baseMods, state_notify_event->latchedMods,
                state_notify_event->lockedMods, state_notify_event->baseGroup,
                state_notify_event->latchedGroup, state_notify_event->lockedGroup);
...
void xkb_init(void) {
    ...
    int success_xkb               = xkb_x11_setup_xkb_extension(
        globalconf.connection, XKB_X11_MIN_MAJOR_XKB_VERSION, XKB_X11_MIN_MINOR_XKB_VERSION, 0,
        NULL, NULL, NULL, NULL);
    ...
    xcb_discard_reply(
        globalconf.connection, xcb_xkb_per_client_flags(
    ...
    xcb_xkb_select_events(
        globalconf.connection, XCB_XKB_ID_USE_CORE_KBD, map, 0, map, map_parts, map_parts, 0);
...
void xkb_free(void) {
    if (globalconf.have_xkb)
        ...
        xcb_xkb_select_events(globalconf.connection, XCB_XKB_ID_USE_CORE_KBD, 0, 0, 0, 0, 0, 0);
    xkb_free_keymap();
}
