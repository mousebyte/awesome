...
bool mouse_query_pointer(
    ...
    *y = query_ptr_r->win_y;
    ...
    if (mask) *mask = query_ptr_r->mask;
    if (child) *child = query_ptr_r->child;
...
static inline void mouse_warp_pointer(xcb_window_t window, int16_t x, int16_t y) {
    xcb_warp_pointer(globalconf.connection, XCB_NONE, window, 0, 0, 0, 0, x, y);
}
...
static int luaA_mouse_index(lua_State *L) {
    ...
    if (A_STRNEQ(attr, "screen")) {
        ...
        } else return luaA_default_index(L);
    }
...
static int luaA_mouse_newindex(lua_State *L) {
    ...
    if (A_STRNEQ(attr, "screen")) {
        ...
        } else return luaA_default_newindex(L);
    }
...
int luaA_mouse_pushstatus(lua_State *L, int x, int y, uint16_t mask) {
    ...
    for (uint16_t maski = XCB_BUTTON_MASK_1; maski <= XCB_BUTTON_MASK_5; maski <<= 1) {
        if (mask & maski) lua_pushboolean(L, true);
...
static int luaA_mouse_coords(lua_State *L) {
    ...
    if (lua_gettop(L) >= 1) {
        ...
        if (!mouse_query_pointer_root(&mouse_x, &mouse_y, NULL, &mask)) return 0;
        ...
        if (ignore_enter_notify) client_ignore_enterleave_events();

        mouse_warp_pointer(globalconf.screen->root, x, y);

        if (ignore_enter_notify) client_restore_enterleave_events();
...
static int luaA_mouse_object_under_pointer(lua_State *L) {
    ...
    if (!mouse_query_pointer_root(&mouse_x, &mouse_y, &child, NULL)) return 0;
