...
void systray_init(void) {
    ...
    xcb_create_window(
        globalconf.connection, xscreen->root_depth, globalconf.systray.window, xscreen->root, -1,
        -1, 1, 1, 0, XCB_COPY_FROM_PARENT, xscreen->root_visual,
        XCB_CW_BACK_PIXEL | XCB_CW_EVENT_MASK,
        (const uint32_t[]) {xscreen->black_pixel, XCB_EVENT_MASK_SUBSTRUCTURE_REDIRECT});
    ...
    if (!atom_name) fatal("error getting systray atom name");
    ...
    if (!atom_systray_r) fatal("error getting systray atom");
...
static void systray_register(void) {
    ...
    if (globalconf.systray.registered) return;
    ...
    xcb_set_selection_owner(
        globalconf.connection, globalconf.systray.window, globalconf.systray.atom,
...
void systray_cleanup(void) {
    ...
    xcb_set_selection_owner(
        globalconf.connection, XCB_NONE, globalconf.systray.atom, globalconf.timestamp);
...
int systray_request_handle(xcb_window_t embed_win) {
    ...
    if (xembed_getbywin(&globalconf.embedded, embed_win)) return -1;
    ...
    xcb_change_window_attributes(
        globalconf.connection, embed_win, XCB_CW_EVENT_MASK, select_input_val);

    /* we grab the window, but also make sure it's automatically reparented back
     * to the root window if we should die.
     */
    xcb_change_save_set(globalconf.connection, XCB_SET_MODE_INSERT, embed_win);
    xcb_reparent_window(globalconf.connection, embed_win, globalconf.systray.window, 0, 0);
    ...
    xembed_embedded_notify(
        globalconf.connection, em.win, globalconf.timestamp, globalconf.systray.window,
...
int systray_process_client_message(xcb_client_message_event_t *ev) {
    ...
    switch (ev->data.data32[1]) {
        case SYSTEM_TRAY_REQUEST_DOCK:
            ...
            if (!(geom_r = xcb_get_geometry_reply(globalconf.connection, geom_c, NULL))) return -1;
...
bool systray_iskdedockapp(xcb_window_t w) {
    ...
    kde_check_q = xcb_get_property_unchecked(
        globalconf.connection, false, w, _KDE_NET_WM_SYSTEM_TRAY_WINDOW_FOR, XCB_ATOM_WINDOW, 0, 1);
...
int xembed_process_client_message(xcb_client_message_event_t *ev) {
    switch (ev->data.data32[1]) {
        case XEMBED_REQUEST_FOCUS:
            xembed_focus_in(
                globalconf.connection, ev->window, globalconf.timestamp, XEMBED_FOCUS_CURRENT);
            break;
    }
    return 0;
}
...
static int systray_num_visible_entries(void) {
    ...
    foreach (em, globalconf.embedded)
        if (em->info.flags & XEMBED_MAPPED) result++;
    return result;
}
...
void luaA_systray_invalidate(void) {
    ...
    luna_emit_global_signal(L, ":systray.update", 0);
...
static void systray_update(
    ...
    xcb_configure_window(
        ...
        XCB_CONFIG_WINDOW_WIDTH | XCB_CONFIG_WINDOW_HEIGHT, config_vals);
    ...
    for (int i = 0; i < globalconf.embedded.len; i++) {
        ...
        if (reverse) em = &globalconf.embedded.tab[(globalconf.embedded.len - i - 1)];
        else em = &globalconf.embedded.tab[i];
        ...
        if (force_redraw) xcb_clear_area(globalconf.connection, 1, em->win, 0, 0, 0, 0);
        if (i % rows == rows - 1) {
            if (horizontal) {
...
int luaA_systray(lua_State *L) {
    ...
    if (lua_gettop(L) == 1) lunaL_drawin_systray_kickout(L);

    if (lua_gettop(L) > 1) {
        size_t      bg_len;
        drawin_t   *w = luaC_checkuclass(L, 1, "Drawin");
        ...
        if (color_init_reply(
                color_init_unchecked(&bg_color, bg, bg_len, globalconf.default_visual)) &&
            globalconf.systray.background_pixel != bg_color.pixel) {
            uint32_t config_back[]              = {bg_color.pixel};
            globalconf.systray.background_pixel = bg_color.pixel;
            xcb_change_window_attributes(
                globalconf.connection, globalconf.systray.window, XCB_CW_BACK_PIXEL, config_back);
            xcb_clear_area(globalconf.connection, 1, globalconf.systray.window, 0, 0, 0, 0);
            force_redraw = true;
        }

        if (globalconf.systray.parent != w)
            xcb_reparent_window(globalconf.connection, globalconf.systray.window, w->window, x, y);
        else {
            uint32_t config_vals[2] = {x, y};
            xcb_configure_window(
                globalconf.connection, globalconf.systray.window,
                XCB_CONFIG_WINDOW_X | XCB_CONFIG_WINDOW_Y, config_vals);
        }
    ...
    luna_object_push(L, globalconf.systray.parent);
    return 2;
}
